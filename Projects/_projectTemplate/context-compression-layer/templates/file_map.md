...
### AI コンテキスト (`_ai-context/`)

| パス | 役割 | 読む優先度 |
|------|------|-----------|
| `_ai-context/project_summary.md` | プロジェクト全体像・背景・目標 | 最初に読む |
| `_ai-context/current_focus.md` | 現在の作業フォーカス・直近の状況 | 毎セッション確認 |
| `_ai-context/decision_log/` | 技術・設計判断の記録 (日付降順で最新3件) | 判断が必要なとき |
| `_ai-context/file_map.md` | このファイル (構造の地図) | 迷ったとき |
...
- 現状把握から始めたい → `_ai-context/project_summary.md` → `_ai-context/current_focus.md`
- 過去の判断を確認したい → `_ai-context/decision_log/` (最新3件)
