...
- Do not directly edit `asana-tasks*.md` files (auto-generated by sync_from_asana.py, Memo area を除く)
- Do not change the structure under `shared/` without authorization
- Do not delete or move junctions

## Asana Integration

各案件の Asana タスクは `_ai-context/obsidian_notes/asana-tasks.md` で参照可能。
セッション開始時、案件の Asana タスクを確認したい場合はこのファイルを読む。
詳細は `_globalScripts/CLAUDE.md` を参照。
