...
| タブ | 機能 |
|------|------|
| Dashboard | プロジェクト一覧 + 直近30日のActivity Bar + Today Queue (折りたたみ式、Asana Done連携) |
| Editor | マルチファイルMarkdownエディタ (AvalonEdit、シンタックスハイライト、エンコーディング自動検出) |
| Timeline | focus_history / decision_log から活動履歴を時系列表示 |
| Git Repos | プロジェクト別 Git リポジトリの一覧・管理 (ブランチ・リモート・最終コミット) |
| Asana Sync | Asanaタスク → Markdown同期 (手動 / 定期自動) |
| Setup | プロジェクト作成・健全性チェック・アーカイブ・Tier変換 (サブタブ: New, Check, Archive, Convert) |
| Settings | テーマ選択・ホットキー設定・Windowsスタートアップ登録・Dashboard設定 (Today Queue表示 / タスク上限 / 自動更新間隔) |

システムトレイ常駐 / グローバルホットキー / Catppuccin Mocha & GitHub Dark テーマ / コマンドパレット (Ctrl+K)

### コマンドパレット (Ctrl+K)

VS Code スタイルの全操作クイックアクセス。タブ切替 (`0`～`9`)、プロジェクト別アクション (`check`、`edit`、`term`、`dir`、`timeline`、`resume`)、エディタショートカット (`@プロジェクト名`)、Morning Briefing コマンド (`briefing`) に対応。

### Morning Briefing

過去72時間の横断サマリーを生成 (完了Asanaタスク・focus更新・decision log)。日付付きMarkdownファイルとして `_ai-workspace/briefings/` に保存し、Editorタブで自動表示。コマンドパレットから `briefing` で起動。
